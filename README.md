...
- **Per-host topic:** a host can send its alerts to its own ntfy topic (*Notification topic* in the host dialog, `ntfyTopic` in the config). Hosts without one, and the machine watchdock runs on, use the global topic. Useful for routing work machines to a team topic and the rest to a personal one.
